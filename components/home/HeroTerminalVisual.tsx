...
export function HeroTerminalVisual() {
  return (
    <motion.div
      ...
      className="lg:col-span-5 relative w-full perspective-2000"
    >
      {/* Background Animated Gradient Halo */}
      <div className="absolute -inset-2 rounded-3xl bg-gradient-to-r from-cyan-500/15 via-emerald-500/10 to-indigo-500/15 blur-2xl opacity-60 pointer-events-none" />

      {/* Floating Badge overlay with 3D elevation */}
      <motion.div
        animate={{ y: [0, -6, 0] }}
        ...
        <div className="relative glass rounded-3xl p-6 sm:p-7 border border-white/15 bg-neutral-950/80 backdrop-blur-2xl shadow-2xl overflow-hidden group">
          ...
          <div className="relative font-mono text-xs text-white/80 space-y-2.5 leading-relaxed overflow-x-auto py-2">
            ...
            <p className="text-emerald-400 flex items-center gap-2 font-medium">
              ...
              <span>await app.deploy(); // Deployed to Global Edge</span>
            </p>
          </div>

          {/* Interactive Feature Pills inside Card with subtle 3D lift */}
